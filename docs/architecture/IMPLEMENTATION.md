...
- **단일 테넌트(의도)**: synub 자사 운영 전용이라 멀티테넌트 SaaS가 아니다. 과거의 `company_id`(항상 1) 컬럼은 혼동을 유발해 **V4에서 제거**(`CurrentTenant`도 삭제). 고객 회사 격리는 `organization`/`org_code`가 담당하며, 조직(회사)이 결제 주체 단위다.
